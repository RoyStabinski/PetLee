...
| 11 | T-24 requires `ApiClient.getMyPets()` and T-32's dashboard is built on it, but `api-contract.md` has no endpoint that returns a user's own listings. `GET /api/pets` cannot stand in: it is the public gallery, so it hides `ADOPTED` and `REMOVED` pets — exactly the ones an owner most needs to see — and `PetDTO` carries no owner field, so a client could not pick its own out of the result either. | **Extend** the contract with `GET /api/pets/mine` — `@Secured`, `200` `List<PetDTO>` of the session user's listings in every status, `401` for a guest. The owner is the session user and never a parameter: an `?ownerId=` would turn the one endpoint that shows a user their withdrawn listings into a way to read anybody's. Nothing existing changes, so no caller breaks. `PetService.findByOwner` (T-15) and `PetRepository.findByOwnerId` (T-08) were written for this and had no caller until now. Implemented in T-24; `api-contract.md` is updated in T-41. | T-24 |
